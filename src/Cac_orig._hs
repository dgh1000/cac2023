...
--------------------------------------------------------------------------------------
type Es st = ExceptT Exc (State (EsState st))


data EsState st = EsState
  { esGen   :: StdGen
  , esState :: st
  }
  deriving(Generic)
...
instance ExcClass (Es st) where
  throwExc msg = throwE $ Exc [msg]
  catchExc = catchE
  withExc = withExceptT


runEs :: Es st a -> StdGen -> st -> (Either Exc a,EsState st)
runEs m gen custom = runState (runExceptT m) (EsState gen custom)


instance Ra (Es st) where
  getGen = gets esGen
  putGen g = modify (\(EsState _ st) -> EsState g st)


--------------------------------------------------------------------------------------
--                     monad Ers for combining MWC random, exceptions, and custom state

data ErsState st = ErsState
  { ersGen   :: StdGen
  , ersState :: st
  }

type Ers st = ExceptT Exc (WriterT [ShowItem] (StateT (ErsState st) Eval))

instance ExcClass (Ers st) where
  throwExc msg = throwE $ Exc [msg]
  catchExc = catchE
  withExc = withExceptT


runErs :: Ers st a -> StdGen -> st -> ((Either Exc a,[ShowItem]),ErsState st)
runErs m gen custom = runEval $ runStateT (runWriterT $ runExceptT m) (ErsState gen custom)


evalErs m gen custom = fst $ runErs m gen custom


baseRandomR :: (Random a,Variate a) => (a,a) -> Ers st a
baseRandomR = baseRandomR_system

{-
baseRandomR_mwc :: (Random a,Variate a) => (a,a) -> Ers st a
baseRandomR_mwc (x,y) = do
  gen <- gets ersGen
  uniformR (x,y) gen
-}

baseRandomR_system :: (Random a,Variate a) => (a,a) -> Ers st a
baseRandomR_system (x,y) = do
  g <- gets ersGen
  let (v,g') = randomR (x,y) g
  modify (\(ErsState _ s) -> ErsState g' s)
  return v

ersRandomR :: (Random a, Variate a) => (a,a) -> Ers st a
ersRandomR r = baseRandomR r



ersChoose :: [a] -> Ers st a
...
ersMaxRandTied :: (a -> Double) -> [a] -> Ers st a
