...
computeMarks :: Map Loc [XMsrData] -> Map Loc [Mark]
computeMarks msrData =
  M.unionWith (++) (lMapMaybeWithKey maybeMetMark msrData)

  where
    metMarks  = lMapMaybeWithKey maybeMetMark msrData
    symMarks  = lMapMaybe        maybeSymbol  msrData
    wordMarks = 
