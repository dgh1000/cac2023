...
module Cac.SimplerB.Search where
...
addNBestSteps :: (RandMonad m, SearchMonad m, CompClass c eu step) =>
                 Int -> c -> m c
addNBestSteps n compIn = do
  let doOne nRemain cIn | nRemain <= 0 = return cIn
                        | otherwise = do
                            cOut <- addBestStep cIn
                            doOne (nRemain-1) cOut
  doOne n compIn


addBestStep :: (RandMonad m, SearchMonad m, CompClass c eu step) => c -> m c
...
--
pickBest :: (RandMonad m, SearchMonad m, CompClass c eu step, Ord step) => 
            [EvalStepScore c step] -> m c
pickBest stepScores =
  (do chosenStep <- rChooseList g1
      ...
      putReport stepReport
      return $ view evaluatedComp chosenStep)
  where
    nonPruned = filter (isJust . view summedResult) stepScores
    nNonPruned = length nonPruned
    PercentileOut _ percSingle = percentile nonPruned ssGetBadness
    leastBadHalf = map snd $ takeWhile ((<= 0.5) . fst) $ percSingle
