...
data State1 = State1 (Map Pcs (Map Int [Pcs]))
...
data Comp1 = Comp1
  ...
  , c1FragSize        :: Int
  , c1Eval            :: [Comp1 -> Ers State1 Double]
  , c1Prune           :: [Comp1 -> Step1 -> Ers State1 Bool]
  }
...
instance Opt Comp1 Step1 State1 where
  ...
  oApply (Step1 s) c  = return c { c1Pits = s : c1Pits c }
...
prune_octRel :: Int -> Comp1 -> Step1 -> Ers State1 Bool
...
prune_octRel' nBack !c !(Step1 s) = do
  let ps = take nBack $ c1Pits c
      isOctRel x = x /= s && x `mod` 12 == s `mod` 12
  return $ any isOctRel ps
  
prune_rep :: Comp1 -> Step1 -> Ers State1 Bool
...
prune_rep' !c !(Step1 s) | null pits = throwExc "prune_rep: null comp"
                         | otherwise = return $ s == head pits
  where pits = c1Pits c


analyzeFrag_er1_1 :: Map Int (Set Pcs) -> Pcs -> Ers State1 (Map Int [Pcs])
analyzeFrag_er1_1 !subsets !frag = do
  (State1 cache) <- gets ersState
  case M.lookup frag cache of
    Just x  -> return x
    Nothing -> do
      let a = analyzeFrag subsets frag
          mod (ErsState g (State1 c)) = ErsState g (State1 $ M.insert frag a c)
      modify mod -- (\s -> s {e1sCache = M.insert frag a cache})
      return a

analyzeFrag_er1_2 subsets frag = return $ analyzeFrag subsets frag


analyzeFrag_er1_3 :: Map Int (Set Pcs) -> Pcs -> Ers State1 (Map Int [Pcs])
...
eval_pcset :: [Double] -> Comp1 -> Ers State1 Double
...
eval_rep :: Int -> [Double] -> Comp1 -> Ers State1 Double
eval_rep !repFragSize !weights !c = do
  let ps = take repFragSize $ c1Pits c
  case ps of
...
data FitFn comp step st = FitFn String (comp -> step -> (Ers st) Double)

data StepResult = StepResult Double Int [Fsr]

evalAll :: Opt comp step st => comp -> [FitFn comp step st] -> [step] -> (Ers st) [AllFsr step]
evalAll c ffs ss = do
  -- ss :: [(step,(Double,[Fsr2]))]
...
evalAll_funcs :: Opt comp step st => comp -> [FitFn comp step st] -> step ->
                 (Ers st) (step,(Double,[Fsr2]))
